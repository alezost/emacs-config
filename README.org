...
- My Emacs start time is always less than a second according to =M-x
  ...
  + I don't load =…-autoloads.el= files for every installed package
    ...
    * one file for my [[file:packages][packages]].
    ...
    see how autoloads are generated.  I also use
    [[file:utils/al-autoload.el][al/generate-autoloads-from-presets]] command to quickly update these
    autoloads when I need.
...
- [[file:settings][settings]]: my configuration for various packages.  These files are
  usually loaded by init files like so:
  ...
  which means load my "dired" file from "settings" directory after
  loading =dired= feature.
...
- [[file:utils][utils]]: various utilities and additional code (for Emacs itself and for
  ...
  features (except for [[file:utils/tui][tui]] packages that have =tui/= prefix and provide
  [[https://github.com/magit/transient][transient]] interface for various things).  These utils are loaded only
  if and when they are needed.  Also there are many interactive commands
  there which are autoloaded.
